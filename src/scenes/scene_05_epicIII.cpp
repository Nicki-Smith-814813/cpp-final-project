...
void scene_05_epicIII(Character& player) {
    ...
    displaySpeakerDialogue("Fates","Fates:'Are you sure about this, Hades? Letting them go? It could unravel everything you've built,'");
    displayDialogue("The Fates hiss like a serpent waiting to strike.");
    displaySpeakerDialogue("Fates","Fates:'You should have kept her here, where she belongs.'");
    ...
    displaySpeakerDialogue("Hades","Hades:'I... I don't know...' he mutters.");
    displaySpeakerDialogue("Hades","Hades:'But if they leave, will the system hold? Will everything fall apart?'");

    displaySpeakerDialogue("Persephone","Persephone:'You will never know unless you give him a chance.'");
    displayDialogue("Persephone says softly but firmly.");
    displaySpeakerDialogue("Persephone","Persephone:'Give him the opportunity. I believe in him.'");

    displayDialogue("Hades sighs heavily, staring at Orpheus.");
    displaySpeakerDialogue("Hades","Hades:'Alright, Orpheus. One last chance. We'll see if you can survive a musical trial.'");
    ...
    if (!orpheusSuccess) {
        dramaticPause(1000);
        displaySpeakerDialogue("Hades", "Hades:'Fine. Try again. But know this — the melody grows longer.'");

        orpheusSuccess = musicalGame(player, true, 2); // Second harder try
    }

    if (orpheusSuccess) {
        displaySpeakerDialogue("Hades", "Hades:'You have proven yourself... for now.'");
    } else {
        // Both normal tries failed
        decreaseFaith(player, 15);
        displaySpeakerDialogue("Fates","Fates:'*You fall again... the thread unravels faster.*'");
        ...
        displaySpeakerDialogue("Fates","Fates:'One. Last. Thread.'");
        dramaticPause(1000);
        ...
        if (choice == 1) {
            ...
            if (fatesChallenge) {
                displaySpeakerDialogue("Hades","Hades:'Fine. You've earned it. Go.");
                increaseFaith(player, 20); // 🌟 Reward Faith for surviving the Fates!
            } else {
                displaySpeakerDialogue("Fates","Fates:*Snapped like a broken string...*");
                displayDialogue("The Fates cackle as the melody collapses into silence.");
            }
        } else {
            displaySpeakerDialogue("Fates","Fates:*Coward... Not even a last stand.*");
